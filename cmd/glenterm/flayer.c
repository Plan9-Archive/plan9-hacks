...
flstart(Rectangle r)
{
	...
	maincols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x55555500);
	maincols[BORD] = allocimage(display, Rect(0,0,2,2), screen->chan, 1, 0xAAAAAA00);
	maincols[TEXT] = display->white;
	maincols[HTEXT] = display->white;

	/* Command text is blueish */
	cmdcols[BACK] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x33333300);
	cmdcols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DPalegreygreen);
	cmdcols[BORD] = allocimage(display, Rect(0,0,2,2), screen->chan, 1, 0xAAAAAA00);
